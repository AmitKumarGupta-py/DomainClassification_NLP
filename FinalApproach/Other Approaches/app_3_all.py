...
def load_artifacts():
    ...
    for model_type in ["domain", "sub_domain"]:
        artifacts[f"classical_models_{model_type}"] = {}
        for model_name in ["Logistic", "SVM", "RandomForest", "XGBoost", "NaiveBayes", "KNN"]:
            with open(f"models/{model_type}_{model_name}.pkl", "rb") as f:
                artifacts[f"classical_models_{model_type}"][model_name] = pickle.load(f)

    # Load CatBoost Models
    for model_type in ["domain", "sub_domain"]:
        with open(f"models/{model_type}_CatBoost.pkl", "rb") as f:
            artifacts[f"catboost_{model_type}"] = pickle.load(f)
    ...
    # Load LSTM models
    for model_type in ["domain", "sub_domain"]:
        artifacts[f"lstm_{model_type}"] = load_model(f"models/lstm_{model_type}.h5")

    # Load BERT models
    artifacts["bert_tokenizer"] = BertTokenizer.from_pretrained("bert-base-uncased")
    for model_type in ["domain", "sub_domain"]:
        artifacts[f"bert_{model_type}"] = BertForSequenceClassification.from_pretrained(f"models/bert_{model_type}")
...
with tabs[0]:
    ...
    if st.button("Predict"):
        ...
        # --- BERT Predictions ---
        bert_tokenizer = artifacts["bert_tokenizer"]
        inputs = bert_tokenizer(processed_text, return_tensors="pt", padding=True, truncation=True, max_length=512)
        with torch.no_grad():
            domain_logits = artifacts["bert_domain"](**inputs).logits
            sub_domain_logits = artifacts["bert_sub_domain"](**inputs).logits
        bert_domain_pred = domain_encoder.inverse_transform([torch.argmax(domain_logits, dim=1).item()])[0]
        bert_sub_domain_pred = sub_domain_encoder.inverse_transform([torch.argmax(sub_domain_logits, dim=1).item()])[0]
        ...
        with st.expander("View Predictions", expanded=True):
            ...
            st.write(f"**Domain Prediction:** {bert_domain_pred}")
            st.write(f"**Sub-Domain Prediction:** {bert_sub_domain_pred}")

print("BERT models integrated successfully in deployment.")
